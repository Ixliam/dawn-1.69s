...
{
   ch->printf( "%s`x", color_itom( txt, ch ).c_str(  ) ); /* changed to ^ from &D */
   return;
}
...
string imcgetname( char_data * ch, string from )
{
   string mud, name;
...
PFUN( imc_recv_tell )
{
   ...
   if( imc_nameof( q->from ) != "ICE" )
   {
      ...
      if( imc_nameof( q->from ) != "*" )
      {
         ...
         IMC_RREPLY_NAME( vic ) = imcgetname( vic, q->from );
      }
   }
   /*
    * Tell social 
   ...
   if( keymap["isreply"] == "2" || keymap["isreply"] == "1" )
      ...
      buf << "`C" << imcgetname( vic, q->from ) << " `cimctells you `c'`W" << keymap["text"] << "`c'`C\r\n";
...
PFUN( imc_recv_emote )
{
   ...
   for( d = first_descriptor; d; d = d->next )
   {
      if( d->connected_state == CON_PLAYING && ( ch = d->original ? d->original : d->character ) != NULL
          && IMCPERM( ch ) >= level )
         imc_printf( ch, "`R[`GIMC`R] %s %s\r\n", imcgetname( ch, q->from ).c_str(  ), keymap["text"].c_str(  ) );
   }
   return;
}
...
PFUN( imc_recv_beep )
{
    ...
    */
   imc_printf( vic, "`c\a%s imcbeeps you.`C\r\n", q->from.c_str(  ) );
}
...
IMC_CMD( imcchanlist )
{
   ...
   imc_printf( ch, "`c%-15s %-15s `B%-15s `b%-7s `C%s\r\n\r\n", "Name", "Local name", "Owner", "Perm", "Policy" );
   for( chn = imc_chanlist.begin(  ); chn != imc_chanlist.end(  ); chn++ )
   {
...
IMC_CMD( imcdisconnect )
{
   ...
   imc_shutdown( false );
   imc_to_char( "IMC2 network disconneted.\r\n", ch );
   return;
}
...
IMC_CMD( imccolor )
{
   ...
   {
      IMCSET_BIT( IMCFLAG( ch ), IMC_COLORFLAG );
      imc_to_char( "`RIMC2 c`Yo`Gl`Bo`Rr `Ris now on. Enjoy :)\r\n", ch );
   }
   return;
}
